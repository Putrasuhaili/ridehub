...
// ✅ Corrected bike rental query using username
$resultR = $conn->query("
    SELECT * 
    FROM rents 
    INNER JOIN bikes ON rents.bikeID = bikes.bikeID 
    WHERE rents.user_id = $user_id
    ORDER BY rents.rent_time DESC 
    LIMIT 1
") or die($conn->error);
?>
    ...
    <a class="navbar-brand" href="../index.html"><img src="../img/logohead80.png" alt="logo"> UMPSA RideHub</a>
        ...
        <div class="collapse navbar-collapse" id="navbarResponsive">
            <ul class="navbar-nav ml-auto">
                <li class="nav-item"><a class="nav-link animate__animated animate__flipInX" href="bicycle_home.php">BICYCLE</a></li>
...
if (isset($_GET['error'])) {
    if ($_GET['error'] == "emptyfields") {
        ...
        renderAlert(
            'You already have an active rental. Please end your current rental before renting another bicycle.',
            'text-warning border-warning'
        );
    }

} else if (isset($_GET['rent']) && $_GET['rent'] == "success") {
    renderAlert('Bicycle rented successfully!', 'text-success border-success');
} else if (isset($_GET['rent']) && $_GET['rent'] == "ended") {
    renderAlert('Your bicycle rental has been successfully ended.', 'text-info border-info');
}
?>
                        ...
                        <div class="col-12">
                            <ul class="nav nav-tabs nav-fill mb-4" id="myTab" role="tablist">
                                ...
                                <li class="nav-item bg-powderblue rounded-top">
                                    <a class="nav-link text-dark" id="bikeInfo-tab" data-toggle="tab" href="#bikeInfo" role="tab" aria-controls="bikeInfo" aria-selected="false">My Bike</a>
                                </li>
                    ...
                    <form action="../includes/update.inc.php" method="post">
                        ...
                        <div class="form-group text-center mt-3">
                            <button type="submit" name="change" class="btn btn-info px-5 font-weight-bold text-white">Change Password</button>
                                    ...
                                    <div class="tab-pane fade" id="bikeInfo" role="tabpanel" aria-labelledby="bikeInfo-tab">
                                            <?php if ($resultR->num_rows > 0): ?>
                                                <?php while($rowR = $resultR->fetch_assoc()): ?>
                                                    <div class="row">
                                                        <div class="col-sm-3 col-md-2 col-5">
                                                            <label class="font-weight-bold">Bike Image</label>
                                                        </div>
                                                        <div class="col-md-8 col-6">
                                                        <img src="../<?php echo htmlspecialchars($rowR['bikeImagePath']); ?>" alt="Bike Image" style="width: 360px; height: 239.5px;" class="img-fluid img-thumbnail">
                                                        </div>
                                                    </div>
                                                    <hr>
                                                    <div class="row">
                                                        <div class="col-sm-3 col-md-2 col-5">
                                                            <label class="font-weight-bold">Bike Price</label>
                                                        </div>
                                                        <div class="col-md-8 col-6">
                                                            RM<?php echo number_format($rowR['bikePrice'], 2); ?>
                                                        </div>
                                                    </div>
                                                    <hr>
                                                    <div class="row">
                                                    ...
                                                    <div class="row">
                                                        ...
                                                        <div class="col-md-8 col-6">
                                                            <?php echo $rowR['bikeDesc']; ?>
                                                        </div>
                                                    </div>
                                                    <hr>
                                                    <div class="form-group col-md-12">
                                                        <p>Bike ID: <?php echo $rowR['bikeID']; ?></p>
                                                        <a href="endrent.inc.php?bikeID=<?php echo $rowR['bikeID']; ?>" class="btn btn-block btn-danger pr-5 pl-5">End My Rent Now</a>
                                                    </div>
                                                <?php endwhile; ?>
                                                    ...
                                                    <?php else: ?>
                                                        <div class="row">
                                                            <div class="col-12 text-center">
                                                                <p>No current bike rental information available.</p>
                                                            </div>
                                                        </div>
                                                    <?php endif; ?>
                                                </div>
...
<div class="modal fade" id="endRentModal" tabindex="-1" role="dialog" aria-labelledby="endRentModalLabel" aria-hidden="true">
		<div class="modal-dialog modal-dialog-centered modal-lg" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">
					<h4> You need to return the bike in 10 work days or else you will be charge on penalty.</h4>
					<h5> Thank You For Renting With Us!</h5>
				</div>
				<div class="modal-footer">
                       <input type="hidden" name="user_id" value="<?php echo $_SESSION['user_id']; ?>">
					<a href="bicycle/endrent.inc.php?bikeID=<?php echo $row['bikeID']?>" class="btn btn-danger pr-5 pl-5">End My Rent Now</a>
